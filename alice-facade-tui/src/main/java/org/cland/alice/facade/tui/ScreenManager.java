 ...
 *
 * <p>对应设计文档 §2 中的 ScreenManager，负责：
 *
 * <ul>
 *   <li>初始化 Lanterna 终端屏幕
 *   <li>管理 UI 组件生命周期
 *   <li>处理键盘输入
 *   <li>驱动渲染循环
 *   <li>处理终端 resize 事件
...
public class ScreenManager implements AutoCloseable {
  ...
  public void start() throws IOException {
    ...
    layout.recalculate(
        terminal.getTerminalSize().getColumns(), terminal.getTerminalSize().getRows());
    renderThread.start();

    // 欢迎消息
    layout.chat().addMessage("System", "Alice Agent v1.0 TUI 已启动。输入 /help 查看可用命令。");
  }
  ...
  /** 处理键盘输入（在主线程中同步调用）。 返回 false 表示请求退出。 */
  ...
  private void handleEnter() {
    ...
    // 检查是否允许提交任务
    if (state.isRunning()) {
      layout.chat().addMessage("System", "Agent 正在执行中，请等待完成或按 F5 停止。");
      return;
    }
