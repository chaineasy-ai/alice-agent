 ...
 * <p>
 * 对应设计文档 §5 数据流图中的 EventBridge 组件。
 * 负责：
 * <ul>
 *   <li>监听 AgentCore 产生的 PPAO 事件</li>
 *   <li>将事件转换为 {@link TuiEvent} 并分发给已注册的监听器</li>
 *   <li>在独立线程中消费事件，不阻塞 UI 渲染循环</li>
 * </ul>
 */
public class EventBridge implements AutoCloseable {
    ...
    public EventBridge() {
        ...
        this.eventThread = Executors.newSingleThreadExecutor(r -> {
