 ...
 * <p>
 * Corresponds to the {@code EnvSnapshot} class in the design document.
 * Contains:
 * <ul>
 *   <li>Timestamp of when the snapshot was taken</li>
 *   <li>Resource versions (URI → version string) tracked via MCP subscriptions</li>
 *   <li>Working directory state (file list, metadata)</li>
 *   <li>Environment variables</li>
 *   <li>List of irreversible side-effects that occurred</li>
 * </ul>
 */
public final class EnvSnapshot {
    ...
    private EnvSnapshot(Builder builder) {
        ...
        this.resourceVersions = builder.resourceVersions != null
            ? Map.copyOf(builder.resourceVersions)
            : Map.of();
        this.workingDirectoryState = builder.workingDirectoryState != null
        ...
        this.environmentVariables = builder.environmentVariables != null
            ...
            : Map.of();
        this.irreversibleEffects = builder.irreversibleEffects != null
            ? List.copyOf(builder.irreversibleEffects)
            : List.of();
    }
    ...
    public Map<String, String> resourceVersions()  { return resourceVersions; }
    public Map<String, Object> workingDirectoryState() { return workingDirectoryState; }
    public Map<String, String> environmentVariables()  { return environmentVariables; }
    public List<IrreversibleSideEffect> irreversibleEffects() { return irreversibleEffects; }
    ...
    public String toString() {
        return "EnvSnapshot{id='" + snapshotId + "', timestamp=" + timestamp
            + ", resources=" + resourceVersions.size()
            + ", files=" + workingDirectoryState.size()
            + ", irreversible=" + irreversibleEffects.size() + "}";
    }

    // ========== Irreversible Side Effect ==========

    /**
     * Records a side-effect that cannot be physically rolled back
     * (e.g., sending an email, posting to social media, executing a financial transaction).
     * <p>
     * When such effects are present, rollback can only be achieved through
     * logical compensation actions (e.g., sending a retraction email).
     */
    public record IrreversibleSideEffect(
        String action,
        String description,
        String compensationSuggestion,
        Instant occurredAt
    ) {
