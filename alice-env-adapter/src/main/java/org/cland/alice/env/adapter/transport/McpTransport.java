 ...
 *
 * <ul>
 *   <li><b>Stdio</b>: local subprocess communication (e.g., Python/Node tools via stdio)
 *   <li><b>SSE</b>: remote server communication via Server-Sent Events streaming
 * </ul>
 *
 * <p>Transport lifecycle: {@link #connect()} → send/receive messages → {@link #disconnect()}.
 */
public interface McpTransport {
   ...
   *
   * <p>For Stdio, this spawns a subprocess and connects to its stdio. For SSE, this establishes an
   * HTTP/SSE connection.
   *
  ...
  /**
   * Register a callback for unsolicited notifications from the server (e.g., resource change
   * events).
   *
