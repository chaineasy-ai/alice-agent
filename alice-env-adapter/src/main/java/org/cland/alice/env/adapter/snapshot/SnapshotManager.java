 ...
 *
 * <p>Corresponds to the {@code SnapshotManager} class in the design document. Uses a deque to store
 * the history of snapshots, supporting {@link #save(EnvSnapshot)} and {@link #rollback()}
 * operations.
 *
 * <p>The rollback strategy follows the "虚实结合" (virtual + physical) approach:
 *
 * <ul>
 *   <li><b>Lightweight attributes</b> — file listings, env vars, resource URIs are captured and
 *       restored logically
 *   <li><b>Physical rollback (Sandbox only)</b> — Docker/Wasm container checkpoint/restore (not
 *       implemented in this class, coordinated externally)
 *   <li><b>Logical compensation</b> — for irreversible side-effects, the snapshot records
 *       compensation suggestions rather than attempting physical rollback
 * </ul>
 */
public final class SnapshotManager {
   ...
   *
   * <p>If the history exceeds {@link #maxHistorySize}, the oldest snapshot is evicted.
   *
   ...
   *
   * <p>Restores the last saved snapshot from history. The snapshot is NOT removed from history on
   * rollback, allowing multiple rollback attempts.
   *
  ...
  public Optional<EnvSnapshot> rollback() {
    ...
    logger.info(
        "Rolling back to snapshot: {} (timestamp: {})", target.snapshotId(), target.timestamp());

    if (target.hasIrreversibleEffects()) {
      logger.warn(
          "Snapshot {} has irreversible side-effects: {}. "
              + "Physical rollback is not possible. "
              + "Consider logical compensation actions.",
          target.snapshotId(),
          target.irreversibleEffects());
    }
  ...
  public Optional<EnvSnapshot> rollbackTo(String snapshotId) {
    ...
    return history.stream().filter(s -> snapshotId.equals(s.snapshotId())).findFirst();
  }

  /**
   * Commit the most recent snapshot as the "good" state.
   *
   * <p>Called when verification passes. The committed snapshot is preserved even if older snapshots
   * are evicted from history.
   */
  public void commit() {
    if (history.isEmpty()) {
   ...
   *
   * <p>Useful for post-action auditing.
   *
  ...
  public static DiffReport diff(EnvSnapshot before, EnvSnapshot after) {
    ...
    for (var effect : after.irreversibleEffects()) {
      if (!before.irreversibleEffects().contains(effect)) {
        builder.addChange("irreversible_effect", effect.action(), null, effect.description());
      }
    }
  ...
  public static final class DiffReport {
    ...
    public java.util.List<DiffEntry> entries() {
      return entries;
    }
    ...
    public record DiffEntry(String type, String key, Object oldValue, Object newValue) {}
