 ...
 * <p>
 * Contains the resource content (as text or structured data), metadata,
 * and the MIME type of the resource.
 */
public final class ResourceResult {
    ...
    public static ResourceResult text(String uri, String mimeType, String text) {
        return builder().uri(uri).mimeType(mimeType).text(text)
            .sizeBytes(text != null ? text.length() : 0).build();
    }
    ...
    public String toString() {
        return "ResourceResult{uri='" + uri + "', mimeType='" + mimeType
            + "', size=" + sizeBytes + "}";
    }
