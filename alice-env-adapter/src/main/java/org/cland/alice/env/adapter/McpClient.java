...
/**
 * MCP 2.0 protocol client — represents a single connection to an external MCP-compatible server
 * (e.g., Filesystem, Database, GitHub).
 *
 * <p>Lifecycle:
 *
 * <ol>
 *   <li>{@link #connect()} — perform MCP handshake, discover capabilities
 *   <li>{@link #listTools()} / {@link #listResources()} — discover capabilities
 *   <li>{@link #callTool(String, Map)} — invoke a tool
 *   <li>{@link #readResource(String)} — read a resource
 *   <li>{@link #disconnect()} — tear down connection
 * </ol>
 */
public final class McpClient {
  ...
  private final AtomicReference<ServerCapabilities> serverCapabilities = new AtomicReference<>();
   ...
   *
   * <p>The handshake includes:
   *
   * <ol>
   *   <li>Transport-level connection
   *   <li>MCP {@code initialize} request (Protocol 2.0)
   *   <li>Capability discovery (tools/list, resources/list)
   * </ol>
   *
    ...
    return transport
        ...
        .thenRun(
            () -> {
              ...
              logger.info(
                  "MCP client '{}' ready with {} tools, {} resources",
                  serverId,
                  tools.size(),
                  resources.size());
            })
  ...
  public CompletableFuture<ToolResult> callTool(String toolName, Map<String, Object> params) {
    ...
    String request =
        buildJsonRpcRequest(
            "tools/call",
            Map.of("name", toolName, "arguments", params != null ? params : Map.of()));

    return transport.send(request).thenApply(this::parseToolResult);
  }
  ...
  public CompletableFuture<ResourceResult> readResource(String uri) {
    ...
    return transport.send(request).thenApply(this::parseResourceResult);
  }
                  ...
                  responseJson -> {
                    try {
                      ...
                      if (resourcesResult != null) {
                        var resourceList =
                            (List<Map<String, Object>>) resourcesResult.get("resources");
