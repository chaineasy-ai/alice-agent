 ...
 * 内部维护：
 * <ul>
 *   <li>{@link JsonSchemaValidator} — 基于 JsonSchema 的结构化数据验证</li>
 *   <li>{@link RegexSafetyFilter} — 基于正则表达式的安全过滤</li>
 * </ul>
 * <p>
 * 用于确定性验证场景（安全策略、权限、数据格式合规性）。
 * 设计为无状态可复用的校验引擎，可被多个 PreValidator / PostValidator 引用。
 */
public final class PolicyEngine {
     ...
     * <p>
     * 支持注册 Schema 定义，并通过 {@link #validate(String, String)} 验证 JSON 数据。
     * 目前使用轻量级模式匹配实现，未来可替换为标准的 JSON Schema 库。
     */
    public static final class JsonSchemaValidator {
         ...
         * <p>
         * 当前实现进行基本的键存在性和类型前缀检查。
         * 未来将对接标准的 JSON Schema 验证库（如 networknt/json-schema-validator）。
         *
        ...
        public List<String> validate(Map<String, Class<?>> schemaRules, Map<String, Object> data) {
            ...
            for (Map.Entry<String, Class<?>> rule : schemaRules.entrySet()) {
                ...
                if (value == null) {
                    ...
                    violations.add("Field '" + rule.getKey() + "' expected type "
                        + rule.getValue().getSimpleName() + " but got "
     ...
     * 基于正则表达式的安全过滤器。
     * <p>
     * 维护一组安全规则（允许/拒绝模式），用于检测输入或输出中
     * 是否包含敏感内容、非法路径、危险指令等。
     * <p>
     * 对应设计文档中 "使用确定性代码逻辑 (Regex, JsonSchema, Policy-as-Code)" 的要求。
     */
    public static final class RegexSafetyFilter {
        ...
        public Optional<String> firstViolation(String content) {
            ...
            for (Pattern deny : denyPatterns) {
                ...
                if (matcher.find()) {
                    return Optional.of("Matched deny pattern: " + deny.pattern()
                        + " at: '" + matcher.group() + "'");
                }
            }
            return Optional.empty();
