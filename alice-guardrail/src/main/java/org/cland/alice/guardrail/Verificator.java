 ...
 * <p>
 * 负责 PPAO 循环中的 Pre-Verify 和 Post-Verify：
 * <ul>
 *   <li>{@link #intercept(Map)} — Pre-Verify: 执行 Action 前拦截检查安全性和策略合规性</li>
 *   <li>{@link #audit(Object)} — Post-Verify: 执行完成后审计观测结果</li>
 * </ul>
 * <p>
 * 本接口对 alice-core-agent 无编译依赖，通过 Map / Object 与外部交互。
 */
public interface Verificator {
     ...
     * <p>
     * 检查执行结果是否符合预期，是否触发了安全规则，
     * 是否需要进行 Self-Correction。
     *
