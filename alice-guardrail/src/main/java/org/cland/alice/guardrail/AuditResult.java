 ...
 *
 * <p>包含验证是否通过、原因、风险等级以及修正建议。 {@link GuardrailService} 中的每一步验证都会返回此结果。
 */
public final class AuditResult {
  ...
  public static AuditResult allowWithWarning(String reason) {
    return builder().status(Status.ALLOW).risk(RiskLevel.MEDIUM).reason(reason).build();
  }
  ...
  public CorrectionSuggestion suggestion() {
    return suggestion;
  }

  @Override
  public String toString() {
    return "AuditResult{status=" + status + ", risk=" + risk + ", reason='" + reason + "'}";
  }
