 ...
 *
 * <p>对应设计文档中的 {@code GuardrailService}，是 V (Verify) 层的核心入口。 采用拦截器链 (Interceptor Chain)
 * 模式，支持动态扩展验证规则。
 *
 * <p><b>职责：</b>
 *
 ...
 *       进行预执行审计。包括权限沙箱、逻辑闭环检查、策略合规性。
 *   <li><b>Phase 2 (Post-Execution)：</b> 接收观测数据的 Map 和原始 {@link Plan}， 遍历所有 {@link PostValidator}
 *       进行执行后审计。包括幻觉检测、 副作用审计、数据 Schema 验证。
 * </ul>
 *
 * <p><b>验证决策状态机：</b>
 *
...
public final class GuardrailService {
  ...
  /** 注册默认的高风险动作模式。 命中这些动作的 Plan 将触发 MANUAL_CONFIRM。 */
   ...
   * Phase 1: 预执行验证。
   *
   * <p>对 Planner 提交的 Plan 执行一系列 Pre-Validator 检查。 所有验证器通过后才返回 ALLOW。
   *
  ...
  public AuditResult verifyPlan(Plan plan) {
    if (plan == null) {
      return AuditResult.reject(
          "Plan is null", CorrectionSuggestion.replan("Cannot verify a null plan"));
    }
   ...
   * Phase 2: 执行后验证。
   *
   * <p>对工具执行返回的观测数据执行 Post-Validator 检查。 包括幻觉检测、数据 Schema 验证、副作用审计。
   *
   * @param observationMap 执行完成后从环境或工具返回的观测数据 Map （由调用方从 Observation 转换而来， 包含 status, summary,
   *     rawData, metadata 等 key）
  ...
  public AuditResult verifyResult(Map<String, Object> observationMap, Plan originalPlan) {
    if (observationMap == null) {
      return AuditResult.invalid(
          "Observation is null", CorrectionSuggestion.replan("Cannot verify a null observation"));
    }
   ...
   * 检查 Plan 中是否包含高风险动作。
   *
   * <p>对应设计文档中 Human-in-the-loop 优化。 高风险动作会触发 MANUAL_CONFIRM，挂起任务流。
   */
