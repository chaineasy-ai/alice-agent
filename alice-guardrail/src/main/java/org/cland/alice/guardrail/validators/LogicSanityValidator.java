 ...
 *
 * <p>对应设计文档中 "逻辑闭环检查" 的具体实现。 验证推理路径是否存在逻辑死循环，以及所需工具的输入是否已在上下文中获得。
 *
 * <p><b>检查规则：</b>
 *
 * <ul>
 *   <li><b>死循环检测：</b>检测 Plan 中是否存在重复的相同 Action 序列模式
 *   <li><b>前置依赖检查：</b>检查步骤间是否存在参数依赖断裂
 *   <li><b>终止保障：</b>确保 Plan 包含明确的终止步骤 (FINISH)
 * </ul>
 */
public final class LogicSanityValidator implements PreValidator {
   ...
   *
   * <p>策略：如果同一 (actionType, target) 连续重复超过 MAX_REPEAT_THRESHOLD 次， 判定为逻辑死循环。
   */
   ...
   *
   * <p>如果最后一步不是 FINISH，则 Plan 可能陷入无限执行。
   */
  private AuditResult ensureTermination(List<Plan.Step> steps) {
    ...
    if (!"FINISH".equals(lastStep.actionType())) {
      ...
      if (!"REVISION".equals(lastStep.actionType())) {
        return AuditResult.reject(
            "Plan lacks termination: last step is "
                + lastStep.actionType()
