 ...
 * 输出渲染器接口。
 *
 * <p>对应设计文档中 {@code OutputRenderer} 接口，负责将 Agent 执行过程中的 步骤结果和最终结论格式化为文本/JSON 输出到标准输出流。
 *
 * <p>两个实现：
 *
 * <ul>
 *   <li>{@link TextOutputRenderer} — 增强文本格式（默认）
 *   <li>{@link JsonOutputRenderer} — JSON 结构化输出（--json）
 * </ul>
 */
