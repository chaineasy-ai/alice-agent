 ...
 * <p>
 * 保留原有 {@code proposeNext(Context)} API 以维持 {@code AgentCore} 的向后兼容性。
 * 内部委托给新的 {@link PlannerService} 双路径决策引擎。
 * <p>
 * 新代码建议直接使用 {@link PlannerService}。
 */
public class ReAct {
    ...
    private ReAct(Builder builder) {
        ...
        // 构建默认模型供应者
        ModelSupplier modelSupplier = builder.modelSupplier != null
        ...
        ThinkingTree tree = builder.tree != null
            ? builder.tree
            : new ThinkingTree(Map.of());

        if (builder.tokenBudget != null) {
            tree.setTokenBudget(builder.tokenBudget);
        }

        SlowPathStrategy slowPath = SlowPathStrategy.builder()
            .tree(tree)
        ...
        // 构建选择器
        StrategySelector.Builder selectorBuilder = StrategySelector.builder()
            .fastPath(fastPath)
            .slowPath(slowPath);
        ...
        // 构建 PlannerService
        this.plannerService = PlannerService.builder()
    ...
    /**
     * 将 Plan 转换为兼容的 Map 意图格式。
     * 如果 Plan 有多个步骤，只返回第一个步骤的意图。
     */
    private Map<String, Object> planToMap(Plan plan, Map<String, Object> context) {
        ...
        return switch (actionType) {
            ...
            case "REVISION" -> {
                ...
                result.put("feedback",
                    firstStep.parameters().getOrDefault("feedback", "Revision requested"));
                yield Map.copyOf(result);
            }
            default -> { // LLM_INFERENCE 及其他
                ...
                result.put("prompt",
                    firstStep.parameters().getOrDefault("prompt",
    ...
    /**
     * 默认的模型供应者实现，使用硬编码的 modelId。
     * 生产环境应替换为通过 alice-model 的 {@code ModelProvider} 获取。
     */
