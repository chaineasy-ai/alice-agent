 ...
 * <p>
 * 负责在接收输入后进行 <b>复杂度评估 (Complexity Assessment)</b>，决定走：
 * <ul>
 *   <li><b>Fast Path (System 1)</b> — 低复杂度任务，直接 LLM 或模板</li>
 *   <li><b>Slow Path (System 2)</b> — 高复杂度任务，MCTS 树搜索</li>
 * </ul>
 * <p>
 * 默认使用 prompt 长度 + 关键词启发式判断复杂度。
 * 可注入自定义 {@code complexityFunction} 实现更精确的判定（如调用 Router 模型）。
 */
public final class StrategySelector {
    ...
    private static final String[] COMPLEX_KEYWORDS = {
        ...
        "plan", "strategy", "multi-step", "complex", "detailed",
        "分析", "比较", "评估", "计划", "策略", "综合"
    };
    ...
    private StrategySelector(Builder builder) {
        ...
        this.complexityFunction = builder.complexityFunction != null
     ...
     */
    public DecisionStrategy fastPath() { return fastPath; }

    /**
     * 获取 SlowPathStrategy 引用（用于测试 / 注入）。
     */
    public DecisionStrategy slowPath() { return slowPath; }
     ...
     * <p>
     * 对应设计文档中轻量化模型（如 Qwen-1.8B）做复杂度判定的替代实现。
     *
     * @param context 上下文快照
    ...
    public static final class Builder {
        ...
        /**
         * 注入自定义复杂度判定函数。
         * 接收上下文快照，返回 true 表示复杂（走 Slow Path）。
         */
