 ...
 * <p>
 * 适用于高复杂度任务，通过 MCTS (Monte Carlo Tree Search) 进行
 * 树搜索与模拟，生成精炼规划。
 * <p>
 * 内部维护 {@link ThinkingTree}，每个 {@link ThinkingNode} 包含
 * State/Action/Value，利用 ForkJoinPool 并行模拟。
 */
public final class SlowPathStrategy implements DecisionStrategy {
    ...
    public Plan decide(Map<String, Object> context) {
        ...
        for (int i = 1; i < bestPath.size(); i++) {
            ...
            planBuilder.addStep(Plan.Step.of(
                node.actionType(),
                node.actionTarget(),
                node.actionParams(),
                node.thought()
     ...
     * 运行 MCTS 树搜索。
     * <p>
     * 通过展开生成候选动作子节点，模拟评估各路径，回溯更新奖励。
     */
    private void runMcts(Map<String, Object> rootState) {
        // 展开器：从叶节点生成候选子节点
        Function<ThinkingNode, List<ThinkingNode>> expander = leaf -> {
            // 基于状态生成候选动作
            List<ThinkingNode> candidates = new ArrayList<>();

            // 候选 1: LLM_INFERENCE
            candidates.add(ThinkingNode.builder()
                .state(rootState)
        ...
        // 模拟器：评估节点状态的奖励
        Function<Map<String, Object>, Double> simulator = state -> {
            // 模拟执行并返回分数
            // 未来可替换为真实模型调用或验证器
