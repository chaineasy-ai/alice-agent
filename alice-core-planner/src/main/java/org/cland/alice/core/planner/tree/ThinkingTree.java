 ...
 * <p>
 * 在 Java 内存中维护，每个 {@link ThinkingNode} 包含 State/Action/Value。
 * 利用 {@link ForkJoinPool} 并行评估多个推理分支，加快 MCTS 的模拟（Simulation）阶段。
 * <p>
 * 支持序列化到 {@code alice-memory-vault}，实现"思维断点"恢复。
 */
public final class ThinkingTree {
    ...
    public ThinkingTree(Map<String, Object> rootState) {
        this.root = ThinkingNode.builder()
            .state(rootState)
            .actionType("ROOT")
            .actionTarget("ROOT")
            .thought("Root node")
    ...
    public ThinkingNode root()                  { return root; }
    public int nodeCount()                      { return nodeCount.get(); }
    public int depth()                          { return depth; }
    public TokenBudget tokenBudget()            { return tokenBudget; }
     ...
     * <p>
     * 对应设计文档中的 expand(ThinkingNode parent)。
     *
    ...
    public void expand(ThinkingNode parent, List<Function<ThinkingNode, ThinkingNode>> childGenerators) {
        ...
        children.computeIfAbsent(parent.nodeId(), k -> new CopyOnWriteArrayList<>())
            .addAll(newChildren);
        parent.markExpanded();
     ...
     * <p>
     * 对应设计文档中的 evaluate(ThinkingNode node)。
     *
     * @param node 待评估节点
     ...
     */
    public void evaluateParallel(List<ThinkingNode> nodes,
                                  Function<Map<String, Object>, Double> evaluator) {
     ...
     * 执行多次 MCTS 迭代。
     */
    public void mctsIterations(int iterations,
                                Function<ThinkingNode, List<ThinkingNode>> expander,
                                Function<Map<String, Object>, Double> simulator) {
    ...
    /**
     * 将树序列化为可存储的扁平结构。
     * 每条记录为：{nodeId, parentId, actionType, actionTarget, reward, visits, expanded}
     */
    ...
    /** 适配展开接口：将 Function<ThinkingNode, List<ThinkingNode>> 转为 List<Function<ThinkingNode, ThinkingNode>> */
    private void expand(ThinkingNode parent,
                         Function<ThinkingNode, List<ThinkingNode>> generator) {
