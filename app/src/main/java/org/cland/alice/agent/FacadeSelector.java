...
public final class FacadeSelector {
   ...
   * 启动 CLI 模式。
   *
   * <p>委托给 {@code org.cland.alice.facade.cmd.AliceCliLauncher}。 如果未提供子命令，打印友好提示并返回成功码。
   */
  private static int launchCli(Agent agent, String[] args) {
    try {
      ...
      if (filteredArgs == null || filteredArgs.length == 0) {
        System.out.println("Alice Agent v" + AliceAgent.VERSION + " - AI-powered autonomous agent");
        System.out.println();
   ...
   * 启动 TUI 模式。
   *
   * <p>委托给 {@code org.cland.alice.facade.tui.AliceTuiLauncher}。
   */
