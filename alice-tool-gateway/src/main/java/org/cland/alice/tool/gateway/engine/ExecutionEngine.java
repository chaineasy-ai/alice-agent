 ...
 * 工具执行引擎 — 核心调度器。
 *
 * <p>对应设计文档类图中 {@code ExecutionEngine}，以及 §3 沙箱执行时序图。
 *
 * <p>职责：
 *
 * <ol>
 *   <li>接收工具名称和参数
 *   <li>通过 {@link ToolRegistry} 查找工具元数据
 *   <li>根据 {@link RiskLevel} 选择沙箱策略
 *   <li>执行工具方法，捕获返回值或异常
 *   <li>包装结果为 {@link ToolResult} 返回
 *   <li>支持超时控制
 * </ol>
 *
 * <p><b>设计说明</b>：本模块不依赖 alice-core-agent 的 {@code Action/Observation} 类型，
...
public class ExecutionEngine {
  ...
  public ToolResult invoke(String toolName, Map<String, Object> params) {
    ...
    try {
      ...
      if (sandbox == null) {
        return ToolResult.failure("No sandbox provider configured for risk level: " + risk);
      }
      ...
      try {
        ...
        return ToolResult.timeout(
            "Tool execution timed out after " + defaultTimeoutMs + "ms: " + toolName);
      }
