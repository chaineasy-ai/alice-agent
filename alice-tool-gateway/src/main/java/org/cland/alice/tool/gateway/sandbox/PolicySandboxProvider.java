 ...
 *
 * <p><b>注意</b>：SecurityManager 在 Java 17+ 已被标记为 deprecated for removal。 此实现主要用于演示。在未来的 Java 版本中建议使用
 * {@link java.lang.ProcessBuilder} 子进程隔离或其他方案替代。
 *
 * @deprecated SecurityManager 机制将在未来 Java 版本中移除， 此实现仅作为参考。生产环境建议使用 Docker/Wasm 沙箱。
 */
@Deprecated
        ...
        case READ_ONLY_TEMP -> {
          if (perm instanceof java.io.FilePermission) {
            ...
            if (path.startsWith(tmpDir)
                && (perm.getActions().equals("read") || perm.getActions().equals("read,write"))) {
              return;
            }
          }
          if (perm instanceof java.net.NetPermission
              || perm instanceof java.lang.RuntimePermission && name.startsWith("setIO")
