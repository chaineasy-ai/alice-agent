 ...
 *
 * <p>对应设计文档 §4.1 动态 JSON Schema 映射。 将 Java 方法的参数列表自动推导为标准 JSON Schema， 以便 LLM Planner 直接读取并生成正确的
 * {@code arguments}。
 *
 * <p>支持类型映射：
 *
 * <ul>
 *   <li>String → {@code type: string}
 *   <li>int / long / Integer / Long → {@code type: integer}
 *   <li>float / double / Float / Double / BigDecimal → {@code type: number}
 *   <li>boolean / Boolean → {@code type: boolean}
 *   <li>List / Set → {@code type: array}
 *   <li>Map / POJO → {@code type: object}
 *   <li>Optional → 提取内部类型并标记 {@code required: false}
 *   <li>enum → {@code type: string} + {@code enum} 约束
 * </ul>
 */
