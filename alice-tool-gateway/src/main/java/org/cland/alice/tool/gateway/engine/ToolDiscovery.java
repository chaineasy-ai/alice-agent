 ...
 * <p>对应设计文档 §2 动态能力注入流程。
 *
 * <p>使用纯反射实现，不依赖 CDI 容器。扫描逻辑：
 *
 * <ol>
 *   <li>接收预实例化的 Bean 列表（由上层传入）
 *   <li>遍历 Bean 的所有 public 方法，筛选带 {@link AgentTool} 的方法
 *   <li>解析方法签名，使用 {@link SchemaGenerator} 生成 JSON Schema
 *   <li>创建 {@link ToolMetadata} 并注册到 {@link ToolRegistry}
 * </ol>
 */
public class ToolDiscovery {
  ...
  /** 从方法中提取参数名称列表。 优先使用 {@link ToolParam#value()}，如果不存在则尝试使用反射参数名。 */
