 ...
 * 工具的完整元数据描述。
 * <p>
 * 对应设计文档类图中的 {@code ToolMetadata}，将 Java 强类型方法
 * 映射为 LLM 可理解的工具原语。包含：
 * <ul>
 *   <li>工具名称与描述（供 Planner 选择工具）</li>
 *   <li>JSON Schema 输入描述（供 LLM 生成正确参数）</li>
 *   <li>{@link MethodHandle} 目标方法引用（供快速反射调用）</li>
 *   <li>风险等级（决定沙箱策略）</li>
 * </ul>
 */
public final class ToolMetadata {
    ...
    private ToolMetadata(Builder builder) {
        ...
        this.targetMethod = Objects.requireNonNull(builder.targetMethod, "targetMethod must not be null");
     ...
     * <ul>
     *   <li>无参数方法 → 直接调用</li>
     *   <li>单参数且为 Map 类型 → 将整个 params map 作为唯一参数</li>
     *   <li>多参数 → 按 {@link #paramNames} 顺序从 map 中提取值</li>
     * </ul>
     *
    ...
    public Object invoke(Map<String, Object> params) throws Throwable {
        ...
        if (paramCount == 1 && targetBean != null
        ...
        if (paramCount == 1
            && targetMethod.type().parameterType(0).isAssignableFrom(Map.class)) {
            if (targetBean != null) {
        ...
        if (targetBean != null && paramCount > 0
            && paramTypes[0].isAssignableFrom(targetBean.getClass())) {
            hasReceiver = true;
        }
        ...
        for (int i = 0; i < paramCount; i++) {
            ...
            String paramName = (paramIdx - startIdx < paramNames.length)
                ? paramNames[paramIdx - startIdx] : "arg" + (paramIdx - startIdx);
    ...
    public MethodHandle targetMethod()            { return targetMethod; }
    public Object targetBean()                    { return targetBean; }
    public RiskLevel riskLevel()                  { return riskLevel; }
    public Class<?> returnType()                  { return returnType; }
    public String[] paramNames()                  { return paramNames; }
