 ...
 * <p>
 * 对应设计文档类图中 {@code ToolRegistry} 的角色，负责：
 * <ul>
 *   <li>存储所有已注册工具的 {@link ToolMetadata} 元数据</li>
 *   <li>按名称查找工具</li>
 *   <li>导出工具列表供 Planner 生成 function calling schema</li>
 * </ul>
 * <p>
 * 设计上对 alice-core-agent 无业务依赖，是一种纯粹的 <b>能力目录</b>。
 * 方便将来将工具部署为独立微服务，通过 MCP 协议挂载。
 */
public class ToolRegistry {
    ...
    public void register(ToolMetadata metadata) {
        ...
        if (previous != null) {
            throw new IllegalArgumentException(
                "Tool already registered: " + metadata.name()
                    + " (existing: " + previous.description() + ", new: " + metadata.description() + ")"
            );
        }
    }
     ...
     * <p>
     * 返回的 Map 列表可直接拼接到 OpenAI/Anthropic 的 tools 参数中。
     *
     ...
     * <p>
     * 查找工具元数据并通过 MethodHandle 直接调用。
     * 无沙箱保护，无超时控制。新代码请使用 {@link org.cland.alice.tool.gateway.engine.ExecutionEngine}。
     *
