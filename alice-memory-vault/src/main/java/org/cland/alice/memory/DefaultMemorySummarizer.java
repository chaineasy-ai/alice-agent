 ...
 * 从原始会话 Trace 中提炼：
 * <ul>
 *   <li><b>Facts</b>：从成功的步骤中提取事实性陈述</li>
 *   <li><b>Success Patterns</b>：从连续成功的步骤序列中提取可复用的模式</li>
 * </ul>
...
public final class DefaultMemorySummarizer implements MemorySummarizer {
    ...
    public Summary summarize(List<Step> trace) {
        ...
        String sessionId = trace.get(0).stepId() != null
    ...
    /**
     * 从 Trace 中提取事实性陈述。
     * 事实 = 成功步骤中 action + input → output 的简洁描述。
     */
    private List<String> extractFacts(List<Step> trace) {
        ...
        for (Step step : trace) {
            ...
            String outputPreview = step.output() != null
                    ? step.output().substring(0, Math.min(100, step.output().length()))
                    : "";
            String fact = "使用 %s 处理 '%s' 得到: %s"
                    .formatted(step.action(), step.input(), outputPreview);
            facts.add(fact);
        }
    ...
    /**
     * 从 Trace 中提取成功模式。
     * 模式 = 连续 MIN_SUCCESS_RUN 个以上成功的步骤序列。
     */
