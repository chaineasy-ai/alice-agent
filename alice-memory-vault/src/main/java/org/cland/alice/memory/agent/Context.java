 ...
 * 表示一次记忆检索的查询上下文。
 *
 * <p>包含原始查询文本、可选的 sessionId、标签、时间范围等， 由 MemoryRouter 分析并路由到合适的记忆 vault。
 */
public final class Context {
  ...
  public boolean isEpisodicQuery() {
    ...
    return q.contains("刚才")
        || q.contains("之前")
        || q.contains("just did")
        || q.contains("previous")
        || q.contains("last step")
        || q.contains("recent")
        || q.contains("最近")
        || q.contains("会话")
        || q.contains("session");
  ...
  public String toString() {
    return "Context{query='%s', sessionId='%s', metadata=%s}".formatted(query, sessionId, metadata);
  }
