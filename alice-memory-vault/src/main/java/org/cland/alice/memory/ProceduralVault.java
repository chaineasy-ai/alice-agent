...
/**
 * 程序记忆（Procedural Memory）Vault。
 * <p>
 * 负责存储"最佳实践"和工具使用 Schema（SOP），
 * 支持模式匹配 / 语义路由，将上下文路由到匹配的 SOP。
 * <p>
 * 对应设计文档：ProceduralVault / SopRegistry 角色。
 * 物理载体建议：YAML / Git / Local Files（当前提供内存实现）。
 * <p>
 * 支持版本控制——当文档中的工具 SOP 更新时，
 * Agent 可通过 CI/CD 自动加载更新后的规范。
 */
public final class ProceduralVault {
    ...
    public ProceduralVault() {
        this(DEFAULT_TOP_K);
    }

    public ProceduralVault(int topK) {
        this.topK = topK;
    }
    ...
    /**
     * 根据上下文匹配最相关的 SOP。
     * 匹配策略：先按 toolName 精确匹配，再按 pattern 关键词匹配。
     *
     * @param ctx 查询上下文
    ...
    public String toString() {
        return "ProceduralVault{sops=%d}".formatted(sops.size());
    }
}
