...
/**
 * 语义记忆（Semantic Memory）Vault 的内存实现。
 *
 * <p>负责存储结构化/非结构化知识（如项目文档、技术手册）， 支持通过向量相似度（HNSW）进行语义检索。
 *
 * <p>对应设计文档：SemanticVault / VectorStore + EmbeddingModel 角色。 物理载体建议：Qdrant / Milvus（当前提供 InMemory
 * 实现，含简易 TF-IDF 检索）。
 *
 * <p>支持 Collection 隔离——Project-C-Land 的私有 API 文档 不应在处理通用技术咨询时被误检索。
 */
public final class InMemorySemanticVault implements SemanticVault {
  ...
  public InMemorySemanticVault() {
    this(DEFAULT_TOP_K, DEFAULT_SIMILARITY_THRESHOLD);
  }

  public InMemorySemanticVault(int topK, double similarityThreshold) {
    this.topK = topK;
  ...
  /**
   * 向知识条目自带的 collection（{@link Knowledge#collection()}）存入一条知识。 如果 collection 为 null 或空，则存入默认
   * collection（"_default"）。
   */
  ...
  public void storeAll(String collection, List<Knowledge> knowledgeList) {
    ...
    collections
        .computeIfAbsent(collection, k -> new CopyOnWriteArrayList<>())
  ...
  /** 计算两段文本的相似度（0.0 ~ 1.0）。 采用简单的词袋 Jaccard 相似度 + 长度归一化。 真实生产环境应替换为 EmbeddingModel 的向量点积/余弦相似度。 */
  ...
  public String toString() {
    ...
    return "InMemorySemanticVault{collections=%d, totalKnowledge=%d}"
