 ...
 *
 * <ul>
 *   <li>动作类型（如 TOOL_CALL、LLM_INFERENCE、OBSERVE）
 *   <li>目标（工具名、模型 ID 等）
 *   <li>参数键值对
 *   <li>关联的 Thought（LLM 的推理过程）
 * </ul>
 */
public final class Action {
  ...
  public static Action llmInference(String modelId, String prompt) {
    return builder().type(Type.LLM_INFERENCE).target(modelId).parameter("prompt", prompt).build();
  }

  /** 快速创建 REVISION 动作 */
  public static Action revision(String feedback) {
    return builder().type(Type.REVISION).target("REVISION").parameter("feedback", feedback).build();
  }
  ...
  public String toString() {
    return "Action{id='" + actionId + "', type=" + type + ", target='" + target + "'}";
  }
