 ...
 * <p>
 * 基于 Vert.x 的 {@link Future} 实现异步 PPAO 闭环：
 * <pre>
 *   Perceive -> Plan -> Verify(Pre) -> Act -> Observe -> Verify(Post) -> Reflect -> (loop | finish)
 * </pre>
 * <p>
 * 对应设计文档中的 AgentExecutor 类，整个 PPAO Loop 被建模为一个 Future 链，
 * 每个阶段接收并传递 {@link AgentContext} 作为状态载体。
 */
public class AgentExecutor {
     ...
     * PPAO 递归循环体。
     * <p>
     * 每一轮迭代执行：Plan -> Verify(Pre) -> Act -> Observe -> Verify(Post) -> Reflect
    ...
    /**
     * 内部记录，将一个 {@link StepResult} 与当前 {@link AgentContext} 关联。
     * 用于在 PPAO 阶段之间传递两者。
     */
    ...
    private Future<StepWithContext> actLlmInference(AgentContext ctx, Action action) {
        ...
        vertx.<StepResult>executeBlocking(() -> {
            try {
    ...
    private Future<StepWithContext> actToolCall(AgentContext ctx, Action action) {
        ...
        vertx.<StepResult>executeBlocking(() -> {
            try {
                boolean success = agentCore.toolRegistry().execute(action.target(), action.parameters());
                if (success) {
    ...
    private Future<StepWithContext> verifyPost(StepWithContext stepWithCtx) {
        ...
        return Future.succeededFuture(
            new StepWithContext(ctx, new StepResult.Continue(revision, Observation.blocked("Post-verify audit failed"))));
    }
    ...
    private Future<AgentContext> reflect(StepWithContext stepWithCtx) {
        ...
        if (stepWithCtx.result() instanceof StepResult.Continue cont) {
            ...
            if (nextAction != null && nextAction.type() == Action.Type.REVISION) {
                String feedback = nextAction.parameters()
                    .getOrDefault("feedback", "No feedback provided").toString();
    ...
    private static Action mapToAction(Map<String, Object> plan) {
        ...
        return switch (type) {
            ...
            case "TOOL_CALL" -> {
                ...
                Map<String, Object> params = (Map<String, Object>) plan.getOrDefault("parameters", Map.of());
