 ...
 * <p>
 * 对应设计文档中 Lifecycle 接口，定义了 Agent 核心决策循环的每个阶段：
 * <ul>
 *   <li>{@link #onPerceive(Object)} — 感知输入，构建上下文</li>
 *   <li>{@link #onPlan(AgentContext)} — 基于上下文规划下一步</li>
 *   <li>{@link #onAct(Action)} — 执行规划的动作</li>
 *   <li>{@link #onVerify(StepResult)} — 验证执行结果并决定后续</li>
 * </ul>
 *
 * @param <I> 输入类型
 */
public interface Lifecycle<I> {
     ...
     * <p>
     * 内部调用 Planner 模块，结合 LLM 推理产生 Thought + Action 对。
     *
     ...
     * 3. Act: 执行指定的 Action。
     * <p>
     * 内部调用 ToolGateway 派发 Action，或调用 ModelProvider 进行 LLM 调用。
     *
     * @param action    要执行的动作
     * @param context   当前上下文
     * @return 执行后的原始观测结果
     ...
     * <p>
     * Pre-verify: 在 Act 前拦截 Action，检测安全性/策略合规性。
     * Post-verify: 在 Observe 后审计结果，触发自省 (Self-Correction)。
     *
